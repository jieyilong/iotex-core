...
RUN mkdir -p $GOPATH/src/github.com/CoderZhi/go-ethereum/ && \
    ...
    make clean build && \
    ln -s ./bin/server /usr/local/bin/iotex-server  && \
    ln -s ./bin/actioninjector /usr/local/bin/iotex-actioninjector && \
    mkdir -p /usr/local/lib/iotex/ && \
    ln -s ./bin/addrgen /usr/local/bin/iotex-addrgen && \
    cp ./crypto/lib/libsect283k1_ubuntu.so /usr/lib/ && \
    cp ./crypto/lib/blslib/libtblsmnt_ubuntu.so /usr/lib/ && \
    mkdir -p /etc/iotex/ && \
    ln -s ./blockchain/testnet_actions.yaml /etc/iotex/testnet_actions.yaml && \
